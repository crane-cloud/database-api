...
def authenticate(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        if kwargs['access_token'] is None:
            raise HTTPException(
                status_code=401, detail="Access token was not supplied")

        try:
            token = kwargs['access_token'].split(' ')[1]
            if (kwargs['access_token'].split(' ')[0] != "Bearer"):
                raise HTTPException(
                    status_code=422, detail="Bad Authorization header. Expected value 'Bearer <JWT>'")
            ...
            raise HTTPException(
                status_code=401, detail="The access token supplied is not a bearer token")
